...
install_slicer_from_website()
{
    ...
    if [ -d slicer ]; then
        ...
        wget -q -O slicer.tar.gz "https://download.slicer.org/download?os=$os&stability=release"
...
install_slicer()
{
    if which brew &> /dev/null; then
        ...
        if [ "$system" == "Darwin" ]; then
            install_slicer_from_website "macosx"
        fi

        if [ "$system" == "Linux" ]; then
            install_slicer_from_website "linux"
        fi
    fi
}
